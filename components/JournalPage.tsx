...
const JournalPage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] pb-32 overflow-x-hidden text-white selection:bg-[#34d399] selection:text-[#0a0a0a]">
            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pt-24 md:pt-40">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pb-10 border-b border-white/10 mb-12">
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#34d399] mb-3">Yureka Editorial</p>
                        <h1 className="text-5xl sm:text-7xl md:text-8xl font-heading font-extrabold text-white leading-[0.9] tracking-tighter">
                            The<br /><span className="text-[#34d399] italic font-serif font-light">Journal.</span>
                        </h1>
                    </div>
                            ...
                            {searchOpen ? (
                                <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 280, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="relative overflow-hidden">
                                    <input
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-full px-5 py-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-[#34d399] transition-colors"
                                    />
                    ...
                    {CATEGORIES.map(cat => (
                        <button
                            ...
                            className={`px-5 py-2 rounded-full text-[11px] font-bold uppercase tracking-widest transition-all ${
                                activeCategory === cat
                                    ? 'bg-[#34d399] text-[#0a0a0a] shadow-lg shadow-[#34d399]/20'
                        ...
                        <Link to={`/blogs/${hero.slug}`} className="group block relative rounded-3xl overflow-hidden aspect-[16/7] shadow-2xl">
                            ...
                            <div className="absolute bottom-0 left-0 p-8 md:p-14 max-w-4xl">
                                <div className="flex items-center gap-3 mb-5">
                                     <span className="bg-[#34d399] text-[#0a0a0a] text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border border-white/10 shadow-lg shadow-[#34d399]/20">{hero.category}</span>
                                 ...
                                 <h2 className="text-3xl sm:text-5xl md:text-6xl font-heading font-extrabold text-white leading-[1.05] tracking-tight mb-5 group-hover:text-[#34d399] transition-colors">
                                     {hero.title}
                                 </h2>
                                 ...
                                 <div className="flex items-center gap-3 mt-6">
                                     <div className="w-8 h-8 rounded-full bg-[#34d399] flex items-center justify-center text-[#0a0a0a] text-sm font-bold shrink-0 border border-white/10">{hero.author?.[0] || 'Y'}</div>
                             ...
                             <div className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center group-hover:bg-[#34d399] transition-all border border-white/10">
                                 <ArrowRight size={18} className="text-white group-hover:translate-x-0.5 transition-transform group-hover:text-[#0a0a0a]" />
                             </div>
                        </Link>
                    </motion.div>
                )}
                            ...
                            >
                                <Link to={`/blogs/${post.slug}`} className="group block">
                                    ...
                                    <div className="space-y-3 px-1">
                                        <h3 className="text-lg font-heading font-bold text-white leading-[1.3] group-hover:text-[#34d399] transition-colors line-clamp-2">{post.title}</h3>
                                        <p className="text-white/40 text-sm font-serif italic line-clamp-2 leading-relaxed">{post.excerpt}</p>
                                        <div className="flex items-center gap-2 pt-1">
                                            <div className="w-5 h-5 rounded-full bg-[#34d399]/20 flex items-center justify-center text-[#34d399] text-[8px] font-bold border border-[#34d399]/20">
                                                {post.author?.[0] || 'Y'}
                                            </div>
                        ...
                        >
                            <Link to={`/blogs/${post.slug}`} className="group block">
                                <div className="relative aspect-[3/2] rounded-xl overflow-hidden mb-4 border border-white/5 shadow-xl">
                                    ...
                                    <span className="absolute bottom-3 left-3 bg-[#34d399]/90 backdrop-blur-sm text-[#0a0a0a] text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-full border border-white/10">
                                        {post.category}
                                    </span>
                                </div>
                                <div className="space-y-2.5 px-0.5">
                                    <h3 className="text-base font-heading font-bold text-white/90 leading-[1.3] group-hover:text-[#34d399] transition-colors line-clamp-2">{post.title}</h3>
                ...
                {filtered.length === 0 && (
                    <div className="py-40 text-center">
                        ...
                        <button onClick={() => { setActiveCategory('All'); setSearchQuery(''); }} className="mt-6 text-[#34d399] text-[11px] font-bold uppercase tracking-widest hover:underline">Clear Filters</button>
                    </div>
                )}
                ...
                <motion.section
                    ...
                    viewport={{ once: true }}
                    className="mt-32 rounded-3xl bg-gradient-to-br from-[#34d399] to-[#059669] p-12 md:p-20 text-center overflow-hidden relative"
                >
                    <div className="absolute -top-20 -right-20 w-64 h-64 bg-white/5 rounded-full blur-3xl" />
                    <div className="absolute -bottom-10 -left-10 w-48 h-48 bg-black/10 rounded-full blur-2xl" />
                    <div className="relative max-w-2xl mx-auto space-y-8">
                        <p className="text-[#0a0a0a]/60 text-[10px] font-bold uppercase tracking-[0.4em]">Weekly Dispatch</p>
                        <h2 className="text-4xl md:text-6xl font-heading font-extrabold text-[#0a0a0a] leading-tight tracking-tight">
                            Stay Ahead of<br />the Credit Curve
                        </h2>
                        <p className="text-[#0a0a0a]/70 text-lg font-serif italic leading-relaxed">
                        ...
                        <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto" onSubmit={(e) => e.preventDefault()}>
                            <input
                                ...
                                className="flex-1 bg-[#0a0a0a]/10 border border-[#0a0a0a]/20 rounded-xl px-5 py-4 text-[#0a0a0a] placeholder:text-[#0a0a0a]/40 outline-none focus:border-[#0a0a0a]/60 transition-colors text-sm"
                            />
                            <button className="bg-[#0a0a0a] text-[#34d399] px-8 py-4 rounded-xl text-[11px] font-bold uppercase tracking-widest hover:bg-white hover:text-[#0a0a0a] transition-all shrink-0">
                                Subscribe
                            </button>
                        </form>
                    </div>
