...
const BottomBanner: React.FC = () => {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[60] h-8 bg-black flex items-center overflow-hidden border-t border-cream/10">
      <div className="flex whitespace-nowrap animate-marquee w-max" style={{ animationDuration: '80s' }}>
        {[...Array(4)].map((_, setIndex) => (
            <React.Fragment key={setIndex}>
                {messages.map((msg, i) => (
                    <div key={`${setIndex}-${i}`} className="flex items-center border-r border-cream/20 h-full">
                        <span className="text-[10px] font-bold text-cream uppercase tracking-widest px-6 py-1">
                            {msg}
                        </span>
                    </div>
                ))}
