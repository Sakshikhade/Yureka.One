    ...
    <div className="w-full h-full flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-[#242424] rounded-b-2xl z-50 flex items-center justify-center">
            <div className="w-10 h-1 bg-cream/10 rounded-full" />
        </div>

        <div className="pt-10 pb-6 px-6 bg-cream/[0.03] backdrop-blur-3xl border-b border-cream/10 flex flex-col items-center gap-3 shrink-0 relative overflow-hidden">
          {/* Subtle Shine */}
          <div className="absolute -top-24 -left-24 w-48 h-48 bg-[#047857]/20 blur-[60px] rounded-full" />
          ...
          <div className="text-center relative z-10">
            <h4 className="text-cream text-[13px] font-medium tracking-tight mb-0.5">The Yield Syndicate</h4>
            <div className="flex items-center justify-center gap-2">
                <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-[pulse_2s_infinite]" />
                <span className="text-cream/40 text-[9px] font-medium uppercase tracking-[0.2em]">
                    3 Members Active
                </span>
            </div>
          </div>
        ...
        >
            ...
            {messages.filter(m => visibleMessages.includes(m.id)).map((msg) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5 px-1">
                    <span className="text-[9px] font-medium uppercase tracking-[0.15em] text-cream/20">{msg.sender}</span>
                </div>
                ...
                {msg.isReel ? (
                  <div className="w-[200px] group cursor-pointer relative">
                    ...
                    <div className="relative rounded-[1.8rem] overflow-hidden border border-cream/10 bg-cream/5 p-2 backdrop-blur-sm">
                        <div className="aspect-[9/16] bg-slate-900 rounded-[1.4rem] relative overflow-hidden">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                <motion.div 
                                    ...
                                    className="w-12 h-12 bg-cream/10 backdrop-blur-xl rounded-full flex items-center justify-center border border-cream/20"
                                >
                                    <Instagram size={24} className="text-cream" />
                                </motion.div>
                            </div>
                            <div className="absolute top-3 left-3 flex items-center gap-2">
                                <div className="px-2 py-0.5 bg-red-500 rounded-md text-[8px] font-bold text-cream tracking-widest animate-pulse">REEL</div>
                            </div>
                        </div>
                        <div className="p-3">
                  ...
                  <div className={`
                    ...
                    ${msg.sender === 'Arjun' 
                        ? 'bg-[#047857] text-cream rounded-tr-[0.4rem] shadow-clay/20' 
                        : 'bg-cream/5 text-cream/90 rounded-tl-[0.4rem] border border-cream/10 backdrop-blur-2xl'
                    }
                  `}>
                    {msg.text}
                  </div>
          ...
          <AnimatePresence>
            {typingUser && (
              ...
              >
                 <span className="text-[9px] font-medium uppercase tracking-widest text-cream/20 mb-2 ml-1 italic">{typingUser} is composing...</span>
                 <div className="bg-cream/5 border border-cream/10 rounded-full px-5 py-2.5 flex gap-1.5 items-center backdrop-blur-xl shadow-xl">
                    <motion.div animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.1, 0.8] }} transition={{ repeat: Infinity, duration: 1.2, delay: 0 }} className="w-1.5 h-1.5 bg-[#047857] rounded-full" />
                    <motion.div animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.1, 0.8] }} transition={{ repeat: Infinity, duration: 1.2, delay: 0.2 }} className="w-1.5 h-1.5 bg-cream/40 rounded-full" />
                    <motion.div animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.1, 0.8] }} transition={{ repeat: Infinity, duration: 1.2, delay: 0.4 }} className="w-1.5 h-1.5 bg-cream/40 rounded-full" />
                 </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        {/* Input Bar */}
        <div className="p-6 pt-2 bg-cream/[0.03] backdrop-blur-3xl border-t border-cream/10 flex items-center gap-4 shrink-0">
          <motion.div whileHover={{ scale: 1.1 }} className="p-2 text-cream/20 hover:text-[#047857]/80 transition-colors cursor-pointer">
            <Plus size={20} />
          </motion.div>
          <div className="flex-1 h-11 bg-cream/5 rounded-2xl px-5 flex items-center border border-cream/5 shadow-inner backdrop-blur-md">
            <span className="text-[11px] text-cream/20 font-medium tracking-wide">Enter the Syndicate...</span>
          </div>
          <motion.div whileHover={{ scale: 1.1 }} className="w-10 h-10 bg-[#047857]/10 text-[#047857] rounded-2xl flex items-center justify-center border border-clay/20 shadow-lg shadow-clay/5 cursor-pointer">
            <Smile size={20} />
          </motion.div>
        </div>

        <div className="h-8 w-full shrink-0 flex items-center justify-center">
           <div className="w-32 h-1.5 bg-cream/10 rounded-full" />
        </div>
      </motion.div>

      {/* Hero Atmosphere Glow */}
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-center pointer-events-none">
          <div className="w-[400px] h-[700px] bg-[#047857]/5 blur-[120px] rounded-full" />
      </div>
      ...
      >
        <div className="bg-cream/80 backdrop-blur-xl p-5 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-ink/5 rotate-3 rounded-2xl w-56">
            <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 bg-[#047857]/10 rounded-xl flex items-center justify-center">
            ...
            <div className="mt-4 pt-4 border-t border-ink/5 flex justify-between items-center">
                ...
                <div className="w-12 h-4 bg-[#047857]/20 rounded-lg" />
            </div>
        </div>
      </motion.div>
    </div>
