    ...
    <header className="sticky top-0 right-0 left-0 h-24 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 z-40 px-4 md:px-10 flex items-center justify-between transition-all duration-300">
      <div className="flex items-center gap-6">
        ...
        <div className="flex flex-col">
          <h1 className="text-2xl md:text-3xl font-heading font-black tracking-tight text-white uppercase leading-none">
      ...
      <div className="flex items-center gap-3 md:gap-5">
        ...
        <div className="flex items-center gap-3 pr-2 md:pr-4 border-r border-white/5 mr-1 md:mr-2">
          <button 
            onClick={() => refreshAll()}
            title="Force Cloud Sync"
            className={`p-2.5 rounded-2xl border border-white/5 hover:bg-white/5 transition-all group overflow-hidden relative ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}
          >
            <RefreshCw size={18} className={`text-white/40 group-hover:text-white transition-colors ${isLoading ? 'animate-spin' : ''}`} />
          </button>
          
          <button 
            onClick={onLogout}
            className="p-2.5 rounded-2xl bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-colors border border-red-500/10 group"
